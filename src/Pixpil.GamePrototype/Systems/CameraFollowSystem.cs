...
internal class CameraFollowSystem : IFixedUpdateSystem {
            ...
            if ( cameraFollow.SecondaryTarget is not null ) {
                if ( cameraFollow.SecondaryTarget.IsDestroyed ) {
                    ...
                    targetPosition = Vector2.Lerp( targetPosition,
                        cameraFollow.SecondaryTarget.GetGlobalTransform().Vector2, 0.35f );
                }
            }
            ...
            if ( !_firstStarted ) {
                ...
                if ( cameraFollow.Style == CameraStyle.Perfect ) {
                    ...
                    if ( MathF.Abs( delta.X ) > deadzone.X ) {
                        lerpedX = Calculator.LerpSnap( cameramanPosition.X,
                            targetPosition.X + deadzone.X * MathF.Sign( delta.X ), lerpAmount, 0.2f );
                    }

                    if ( MathF.Abs( delta.Y ) > deadzone.Y ) {
                        lerpedY = Calculator.LerpSnap( cameramanPosition.Y,
                            targetPosition.Y + deadzone.Y * MathF.Sign( delta.Y ), lerpAmount, 0.2f );
                    }
    ...
    private static Vector2 ClampBounds( int width, int height, Camera2D camera, Vector2 position ) {
