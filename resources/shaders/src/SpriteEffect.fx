...
float4 PS(PSInput input, uniform const bool gammaCorrection) : SV_Target0
{
  ...
  // input.TexCoord.x = input.TexCoord.x + Dither1(input.TexCoord);
  // input.TexCoord.y = input.TexCoord.y - Dither1(input.TexCoord);
  float4 color = tex2D(TextureSampler, input.TexCoord);
  // color.r = Noise2(color.rg);
  // color.g = Noise2(color.gb);
  // color.b = Noise2(color.br);
